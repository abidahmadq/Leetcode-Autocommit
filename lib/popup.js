...
function updateSolution(solution) {
  ...
  for (i = 0; i < solution.tags.length; i++) {
    ...
    var readMepath = '/repos/' + currentRepo + '/contents/LeetCodeSolutions/' + solution.difficulty + '/' + solution.tags[i] + '/' + solution.title + '/README.md';
    updateContent(path, {
      message: solution.speed,
      content: encode(solution.content)
    }, function () {

      createQuestionReadme(readMepath, solution.title, solution.description);
